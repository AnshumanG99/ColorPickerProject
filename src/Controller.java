...
public class Controller extends Sprite
{
	...
	private boolean isOverLeftButton(int x, int y)
	{
		...
		double radius = myButtonSize*.5;
		double xdistance = Math.pow(x-myLeftButton.x, 2);
		double ydistance = Math.pow(y-myLeftButton.y, 2);
	...
	private boolean isOverRightButton(int x, int y)
	{
		// TODO: Replace this with the appropriate implementation.
		double radius = myButtonSize*.5;
		double xdistance = Math.pow(x-myRightButton.x, 2.0);
		double ydistance = Math.pow(y-myRightButton.y, 2.0);
		double distance = Math.pow(xdistance + ydistance, 0.5);
		return distance < radius;
	}
	...
	public void act()
	{
		if (myLeftIsPressed && myShade-STEP >= 0) {
			myShade -= STEP;
		}
		if (myRightIsPressed && myShade+STEP <= 255) {
			myShade += STEP;
		}
		// TODO: Replace this with the appropriate implementation.
	}
	...
	public Color getColor()
	{
		...
		if (myHue.equals(RGBA.RED)) {
			rVal = myShade;
		}
		else if (myHue.equals(RGBA.GREEN)) {
			gVal = myShade;
		}
		else if (myHue.equals(RGBA.BLUE)) {
			bVal = myShade;
		}
		else {
			aVal = myShade;
